...
export default function TaskSimulation() {
    ...
    const renderMarkdown = (text) => {
        return text.split('\n').map((line, i) => {
            if (line.startsWith('### ')) {
                return <h3 key={i} className="text-base font-semibold text-gray-900 dark:text-white mt-4 mb-2">{line.replace('### ', '')}</h3>;
            }
            if (line.startsWith('## ')) {
                return <h2 key={i} className="text-lg font-bold text-gray-900 dark:text-white mt-6 mb-3">{line.replace('## ', '')}</h2>;
            }
            if (line.startsWith('* ')) {
                return <li key={i} className="text-sm text-gray-700 dark:text-gray-300 ml-4 mb-2">{line.replace('* ', '')}</li>;
            }
            if (line.startsWith('1. ')) {
                return <li key={i} className="text-sm text-gray-700 dark:text-gray-300 ml-4 mb-2" style={{ listStyleType: 'decimal' }}>
                    {line.replace(/^\d+\.\s/, '')}
                </li>;
            }
            if (line.trim()) {
                return <p key={i} className="text-sm text-gray-700 dark:text-gray-300 mb-3">{line}</p>;
            }
            return null;
        });
    };
        ...
        <div className="flex h-[calc(100vh-8rem)] flex-col gap-4">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-dark-700">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" className="gap-2" onClick={handleSubmit} disabled={submitting || submitted}>
                        {submitting ? (
            ...
            {submitted && (
                <Card className="bg-green-900/20 border-green-800 border-2">
                    <CardContent className="pt-4 flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-medium text-black">Task submitted successfully!</p>
                            <p className="text-sm text-black">Your code will review shortly.</p>
                        </div>
                    </CardContent>
                </Card>
            )}
                    ...
                    <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
                        {activeTab === 'description' ? (
                            ...
                            <div className="space-y-4">
                                <div>
                                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Learning Resources</h3>
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 pt-4">Attached Files</h3>
                                    <div className="flex gap-2 flex-wrap">
                                        <Badge variant="outline" className="cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-700">auth.config.js</Badge>
                                        <Badge variant="outline" className="cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-700">package.json</Badge>
                                    </div>
                                </div>
                            </div>
                        )}
